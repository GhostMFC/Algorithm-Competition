...
signed main(){
  ...
  v.insert({m,n});//n (m)s
  for(int i=1;i<=k;++i){
    auto pos=v.lower_bound({(1<<a[i]),(1<<a[i])});
    if(pos==v.end()||(*pos).second<(1<<a[i])){
      cout<<"No\n";
      return 0;
    }
    ll yuan=(*pos).first;
    ll nwy=(*pos).second-(1<<a[i]),nwx=(*pos).first-(1<<a[i]);
    v.erase(pos);
    if(nwx) v.insert({nwx,(1<<a[i])});
    if(nwy) v.insert({yuan,nwy});
  }
  cout<<"Yes\n";
