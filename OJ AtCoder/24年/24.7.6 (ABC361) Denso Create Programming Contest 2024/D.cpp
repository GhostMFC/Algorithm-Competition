...
inline void bfs(){
  ...
  f[s][n]=0;
  while(q.size()){
    int c=q.front().first,empty=q.front().second; q.pop();
    for(int i=0;i<n+1;++i){
      if(i==empty||i+1==empty||i==empty+1) continue;
      int a=((1<<i)&c),b=((1<<(i+1))&c);
      int to=c-((1<<i)&c)-((1<<(i+1))&c)+((1<<empty)*(!!a))+(1<<(empty+1))*(!!b);
      if(f[to][i]>f[c][empty]+1) f[to][i]=f[c][empty]+1,q.push({to,i});
    }
  }
}
