...
int a[N],b[N],c[N<<1],len,o[N<<2][N<<2][N<<2];
...
signed main(){
  ...
  IOS
  //int T;
  for(int i=1;i<=6;++i) cin>>a[i],c[++len]=a[i];
  for(int i=1;i<=6;++i) cin>>b[i],c[++len]=b[i];
  sort(c+1,c+1+len);
  len=unique(c+1,c+1+len)-c-1;
  for(int i=1;i<=6;++i) a[i]=lower_bound(c+1,c+1+len,a[i])-c;
  for(int i=1;i<=6;++i) b[i]=lower_bound(c+1,c+1+len,b[i])-c;
  for(int i=a[1]+1;i<=a[4];++i)
    for(int j=a[2]+1;j<=a[5];++j)
      for(int k=a[3]+1;k<=a[6];++k)
        o[i][j][k]=1;
  for(int i=b[1]+1;i<=b[4];++i)
    for(int j=b[2]+1;j<=b[5];++j)
      for(int k=b[3]+1;k<=b[6];++k)
        if(o[i][j][k])
          return cout<<"Yes\n",0;
  cout<<"No\n";
  return 0;
