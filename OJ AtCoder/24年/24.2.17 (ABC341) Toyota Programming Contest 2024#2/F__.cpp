...
int n,m,_in[N];
vector<int> G[N],G2[N];
ll a[N],w[N],f[N],dp[N][N];
inline void Add(int fo,int to)
{ G[fo].pb(to),G2[to].pb(fo);}
...
inline void get_dp(int pos){
  ...
  for(int x:G2[pos]){
    int wx=w[x],vx=f[x];
    for(int i=w[pos]-1;i>=wx;--i)
      sMax(dp[pos][i],dp[pos][i-wx]+f[x]);
  } 
...
signed main(){
  ...
  for(PII t:tv){
    int fo=t.first,to=t.second;
    if(w[fo]<w[to]) Add(fo,to),_in[to]++;
    else if(w[fo]>w[to]) Add(to,fo),_in[fo]++;
  }//directed graph
  solve();
