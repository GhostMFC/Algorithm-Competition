...
inline void dfs(int x,int y,int num,int d,int tot){
  if(tot>4) return ;
  ans[x][y]=num;
  int tx,ty;
  tx=x+drx[d],ty=y+dry[d];
  if(!tx||!ty||tx>n||ty>n||ans[tx][ty]) dfs(x,y,num,(d+1)%4,tot+1);
  else dfs(tx,ty,num+1,d,0);
}
