...
inline bool check(int pos,ll add){
  ll res=k-add;
  ll top=p[pos].v+add;
  ...
  while(l<r){
    int mid=(l+r+1)>>1;
    if(p[mid].v>top) l=mid;
    else r=mid-1;
  }

  ll need=(top+1)*(m-l)-(sum[m]-sum[l]);
  if(pos<=m) need=need+p[pos].v-p[m+1].v;
  if(need>res) return true;
  // if(pos==1&&add==74) cout<<l<<' '<<need<<'\n';
  return false;
}
...
signed main(){
  ...
  for(int i=1;i<=n;++i){
    ...
    while(l<r){
      ...
      // if(i==1) cout<<mid<<' '<<check(i,mid)<<' '<<l<<' '<<r<<'\n';
    }
    ans[p[i].num]=(l>k?-1:(l<0?0:l));
  }
  for(int i=1;i<=n;++i)
    cout<<ans[i]<<' ';
  cout<<'\n';
