...
inline int dfs(int pos,vector<int> f){
	if(pos<0) return 0;
	vector<int> s,t;
	for(auto i:f){
		if(i&(1<<pos)) t.push_back(i);
		else s.push_back(i);
	}
	
	if(s.empty()) return dfs(pos-1,t);
	if(t.empty()) return dfs(pos-1,s);
	//break into the smaller fields
	return Min(dfs(pos-1,t),dfs(pos-1,s))|(1<<pos);
}
