...
int n,m,mx,a[maxn<<1],cnt;
int fo[maxn],to[maxn],fa[maxn<<1];
...
//  freopen("C.out","w",stdout);
  ...
  cin>>n;
	for(int i=1;i<=n;++i){
  	cin>>fo[i]>>to[i];
  	a[++cnt]=fo[i],a[++cnt]=to[i];
	} a[++cnt]=1;
	sort(a+1,a+1+cnt);
	cnt=unique(a+1,a+1+cnt)-a-1;
	
	for(int i=1;i<=n;++i){
		fo[i]=lower_bound(a+1,a+1+cnt,fo[i])-a;
		to[i]=lower_bound(a+1,a+1+cnt,to[i])-a;
	}
	...
	for(int i=1;i<=n;++i){
		int f1=find(fo[i]),f2=find(to[i]);
		if(f1==f2) continue;
		fa[f2]=f1;
	}
	
	int mx=1,fafa=find(1);
	for(int i=1;i<=n+n;++i)
		if(find(i)==fafa) mx=i;
