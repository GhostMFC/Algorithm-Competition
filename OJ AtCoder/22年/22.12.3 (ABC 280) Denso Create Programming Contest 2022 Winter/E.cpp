...
inline void solve(){
  cin>>n>>p; //scanf("%d%d",&n,&m);
  
  ll k1=p*q_Pow(100,mod-2,mod)%mod,
  k2=(100-p)*q_Pow(100,mod-2,mod)%mod;
  val[n]=1;
  ll ans=0;
  for(int i=n;i;--i){
  	if(i>1)
  		val[i-2]=(val[i-2]+val[i]*k1)%mod;
  	else
  		val[i-1]=(val[i-1]+val[i]*k1)%mod;
  	val[i-1]=(val[i-1]+val[i]*k2)%mod;
	}
	for(int i=0;i<=n;++i)
		ans=(ans+val[i])%mod;
	cout<<ans-1<<'\n';
