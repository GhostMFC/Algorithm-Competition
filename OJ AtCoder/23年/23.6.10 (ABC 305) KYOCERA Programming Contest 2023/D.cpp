...
signed main(){
  ...
  cin>>n;
  for(int i=1;i<=n;++i)
    cin>>a[i],b[i]=b[i-1]+(a[i]-a[i-1])*(i&1);
  ...
  while(q--){
    ...
    int l,r;
    cin>>l>>r;
    int pos1=lower_bound(a+1,a+1+n,l)-a;
    int pos2=lower_bound(a+1,a+1+n,r)-a;

    if(a[pos2]>r) ans+=(pos2&1)*(r-a[pos2-1]),pos2--;
    if(a[pos1]>l) ans+=(a[pos1]-l)*(pos1&1);
    ans+=b[pos2]-b[pos1];
