...
{ while(x<=v.size()) tr[x]+=1,x+=lowbit(x);}
...
signed main(){
  ...
  //int T;
  cin>>n>>m;
  for(int i=1;i<=n;sort(node[i].a+1,node[i].a+1+m),++i)
    for(int j=1;j<=m;++j)
      cin>>node[i].a[j],v.push_back(node[i].a[j]);
  
  sort(All(v));
  for(int i=1;i<=n;++i)
    for(int j=1;j<=m;++j)
      node[i].a[j]=lower_bound(v.begin(),v.end(),node[i].a[j])-v.begin()+1;

  for(int i=1;i<=m;++i) Add(node[n].a[i]);
  for(int i=n-1;i;--i){
    for(int j=1;j<=m;++j){
      ans+=1ll*j*(n-i);
      ans+=Sum(node[i].a[j]-1);
    }
    for(int j=1;j<=m;++j)
      Add(node[i].a[j]);
  }
