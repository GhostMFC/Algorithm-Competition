...
ll n,x,p[N],t[N],y,q,llcm;//1<=p<=8,t<=1e9

map<pair<int,int>,ll> f[N];

inline ll dfs(ll time,int pos){
  if(pos==n) return 0;
  if(f[pos][{time%p[pos],time%llcm}]) return f[pos][{time%p[pos],time%llcm}];

  ll rst=time%p[pos];
  f[pos][{time%p[pos],time%llcm}]=dfs(time+(rst!=0)*(p[pos]-rst)+t[pos],pos+1)+t[pos]+(rst!=0)*(p[pos]-rst);
  return f[pos][{time%p[pos],time%llcm}];
}

inline void solve(){

  for(int i=0;i<llcm;++i) dfs(i,1);
...
signed main(){
  ...
  //int T;
  read(n,x,y);
  for(int i=1;i<n;++i) read(p[i],t[i]);
  
  llcm=p[1];
  for(int i=2;i<n;++i) llcm=llcm*p[i]/Gcd(llcm,p[i]);//llcm(max)=840
