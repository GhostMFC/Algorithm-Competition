...
inline void solve(int m){
  memcpy(t,s,sizeof(t));
  for(int i=m+1;i<=n;++i) t[(i-1)%m+1]|=t[i];
  int tot=0; ll p=0;
  for(int i=1;i<=m;++i) tot+=(t[i]^1);//前m天未确定的工作日数
  for(int i=1;i*i<=m;++i) if(m%i==0){
    //重复周期
    p-=res[i];//m限制应比因数更少,所以因数能形成的t一定被包涵在m方案中
    if(i*i!=m) p-=res[m/i];
    p=(p%mod+mod)%mod;
  } p=(p+q_Pow(2,tot,mod))%mod,res[m]=p,ans=(ans+p)%mod;
}
