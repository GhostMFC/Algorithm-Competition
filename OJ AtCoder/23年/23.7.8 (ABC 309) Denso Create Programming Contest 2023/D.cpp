...
signed main(){
  ...
  dis[0][1]=0,dis[1][n[0]+n[1]]=0;
  while(q.size()){
    int pos=q.front(); q.pop();
    for(int i=head[pos];i;i=edge[i].nxt){
      int &to=edge[i].to;
      if(dis[0][to]>dis[0][pos]+1){
        dis[0][to]=dis[0][pos]+1;
        q.push(to);
      }
    }
  }
  ...
  while(q.size()){
    ...
    for(int i=head[pos];i;i=edge[i].nxt){
      int &to=edge[i].to;
      if(dis[1][to]>dis[1][pos]+1){
        dis[1][to]=dis[1][pos]+1;
        q.push(to);
      }
    }
  }

  cout<<1+(*max_element(dis[0]+1,dis[0]+1+n[0])+*max_element(dis[1]+1+n[0],dis[1]+1+n[0]+n[1]))<<'\n';
