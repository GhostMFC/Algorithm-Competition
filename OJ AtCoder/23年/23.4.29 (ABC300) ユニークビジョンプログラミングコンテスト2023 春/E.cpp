...
signed main(){
  ...
  //由2,3,5乘出的大数
    ...
    ll pp=q_Pow(5,mod-2,mod);
    dp[0][a[2]][a[3]][a[5]]=1;
    for(int i=1;i<=a[2]+a[3]+a[5]+1;++i){
      for(int j=0;j<=a[2];j++)
        for(int k=0;k<=a[3];++k)
          for(int v=0;v<=a[5];++v){
            dp[i][j][k][v]=(dp[i][j][k][v]+dp[i-1][j+1][k][v]*pp%mod)%mod;//2
            dp[i][j][k][v]=(dp[i][j][k][v]+dp[i-1][j+2][k][v]*pp%mod)%mod;//4
            dp[i][j][k][v]=(dp[i][j][k][v]+dp[i-1][j+1][k+1][v]*pp%mod)%mod;//6
            dp[i][j][k][v]=(dp[i][j][k][v]+dp[i-1][j][k+1][v]*pp%mod)%mod;//3
            dp[i][j][k][v]=(dp[i][j][k][v]+dp[i-1][j][k][v+1]*pp%mod)%mod;//5
          }
    }
    ll ans=0;
    for(int i=1;i<=a[2]+a[3]+a[5]+1;++i){
      ans=(ans+dp[i][0][0][0])%mod;
    }
    cout<<ans<<'\n';
  }
