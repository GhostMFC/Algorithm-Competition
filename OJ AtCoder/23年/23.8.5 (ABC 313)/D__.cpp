...
signed main(){
  ...
  for(int i=k+1;i<=n;++i) get(i);
  for(int i=1;i<k;++i) get2(i);

  a[k]=0;
  for(int i=k+1;i<=n;++i)
    a[i]=a[k]^f1k^fki[i];
  for(int i=1;i<k;++i)
    a[i]=a[k]^fki[k+1]^fki[i];
  
  int ck=0;
  for(int i=1;i<=k;++i)
    ck^=a[i];
  
  if(ck!=f1k) for(int i=1;i<=n;++i) a[i]^=1;
  cout<<"!"; for(int i=1;i<=n;++i) cout<<' '<<a[i]; cout<<'\n';
  cout.flush();  
