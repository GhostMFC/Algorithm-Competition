...
signed main(){
  ...
  cin>>n>>a>>b>>p>>q;
  f[a][b][0]=1;
  Pp=q_Pow(p,mod-2,mod);
  Pq=q_Pow(q,mod-2,mod);
  ...
  for(int i=0;i<2*n;++i)
    for(int j=a;j<n;++j)
      for(int k=b;k<n;++k){
        if((i+1)&1){
          for(int o=1;o<=p;++o)
            f[Min(j+o,int(n))][k][i+1]=
              (f[Min(j+o,int(n))][k][i+1]+Pp*f[j][k][i]%mod)%mod;
        } else{
          for(int o=1;o<=q;++o)
            f[j][Min(k+o,int(n))][i+1]=
              (f[j][Min(k+o,int(n))][i+1]+Pq*f[j][k][i]%mod)%mod;
        }
      }
