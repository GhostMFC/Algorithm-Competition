...
inline void bfs(int x,int rt){
  queue<int> q;
  q.push(x);
  dis[rt][x]=0,val[rt][x]=a[x];
  while(q.size()){
    int fo=q.front(); q.pop();
    for(int to:G[fo]){
      if(dis[rt][to]==-1){
        dis[rt][to]=dis[rt][fo]+1,val[rt][to]=val[rt][fo]+a[to];
        q.push(to);
      } else if(dis[rt][to]==dis[rt][fo]+1){
        if(val[rt][to]<val[rt][fo]+a[to])
          val[rt][to]=val[rt][fo]+a[to];
      }
    }
  }
}
