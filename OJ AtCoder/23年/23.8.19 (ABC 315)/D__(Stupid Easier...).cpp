...
signed main(){
  ...
  for(int i=1;i<=n;++i)
    for(int j=1;j<=m;++j)
      cin>>c[i][j],tx[i][c[i][j]-'a']++,ty[j][c[i][j]-'a']++;
  ...
  while(tag){
    ...
    for(int i=1;i<=n;++i){
      int tot=0,tt=0;
      for(int j=0;j<26;++j)
        if(tx[i][j]) tt++,tot+=tx[i][j];
      if(tt==1&&tot>1) x.pb(i);
    }
    for(int i=1;i<=m;++i){
      int tot=0,tt=0;
      for(int j=0;j<26;++j)
        if(ty[i][j]) tt++,tot+=ty[i][j];
      if(tt==1&&tot>1) y.pb(i);
    }

    for(int a:x)
      for(int i=1;i<=m;++i) if(c[a][i])
        tx[a][c[a][i]-'a']--,ty[i][c[a][i]-'a']--,c[a][i]=0;
    for(int a:y)
      for(int i=1;i<=n;++i) if(c[i][a])
        tx[i][c[i][a]-'a']--,ty[a][c[i][a]-'a']--,c[i][a]=0;
    tag=x.size()+y.size();
  }
