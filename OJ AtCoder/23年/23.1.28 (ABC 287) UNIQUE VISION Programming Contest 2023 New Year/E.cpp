...
inline void get_string(int num){
  cin>>s+1;
  int len=strlen(s+1);
  int pos=0;
  for(int i=1;i<=len;++i){
    node[pos].tot++;
    if(!node[pos].son[s[i]-'a'])
      node[pos].son[s[i]-'a']=++cnt;
    pos=node[pos].son[s[i]-'a'];
  } node[pos].tot++,node[pos].num.push_back(num);
}

inline void dfs(int x,int res){
  for(int i=0;i<26;++i){
    if(node[x].son[i])
      dfs(node[x].son[i],node[node[x].son[i]].tot>1?res+1:res);
  }
  for(int y:node[x].num)
    ans[y]=res;
}
