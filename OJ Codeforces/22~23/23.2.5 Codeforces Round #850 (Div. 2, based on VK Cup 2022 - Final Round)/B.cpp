...
signed main(){
  ...
  while(T--){
    ...
    ll dl=-1,dr=-1,mxr=llinf,mxl=llinf;

    for(int i=1;i<=n;++i){//每一块只能对准一个
      if(a[i]-w+h>b[i])
        dl=Max(dl,(a[i]-w+h)-b[i]);//<--
      else
        dr=Max(dr,b[i]-(a[i]+w-h));//-->
      mxl=Min(mxl,b[i]-(a[i]-w+h)),mxr=Min(mxr,(a[i]+w-h)-b[i]);
    }



    if(dl!=-1&&dr!=-1) goto No;
    else if(dl!=-1){
      if(dl>mxr) goto No;
    } else if(dr!=-1){
      if(dr>mxl) goto No;
