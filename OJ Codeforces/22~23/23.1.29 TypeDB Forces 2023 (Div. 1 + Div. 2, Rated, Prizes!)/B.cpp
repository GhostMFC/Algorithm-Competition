...
inline void solve(){
	...
	if(tmp!=1) v.push_back({1,tmp});
	
	int pos=v.size(),ans=0,k=1,lst=0;
	for(auto x:v) k*=x.second;
	...
	while(pos){
		int l=v[pos-1].first;//¸öÊý 
		ans=ans+k*(l-lst);
		lst=v[pos-1].first;
		while(v.size()&&v[pos-1].first==lst)
			k/=v[--pos].second,v.pop_back();
	}
	cout<<ans<<'\n';
}
